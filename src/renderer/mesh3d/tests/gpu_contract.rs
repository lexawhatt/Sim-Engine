...
pub(in crate::renderer) fn assert_gpu_depth_contract(
    ...
    instancing::assert_gpu_batch_contract(device, queue, format);
    edge_upload::assert_gpu_edge_upload_contract(device, queue, format);
    frame_upload::assert_gpu_dirty_frame_upload_contract(device, queue, format);
    dynamic::assert_gpu_dynamic_contract(device, queue, format);
    vertex_color::assert_gpu_vertex_color_contract(device, queue, format);
    materials::assert_gpu_material_contract(device, queue, format);
    illumination::assert_gpu_lighting_contract(device, queue, format);
    material_resource::assert_gpu_sort_budget(device, queue, format);
    color_budget::assert_gpu_color_budget(device, queue, format);
    ...
    native_acceptance::assert_gpu_native_acceptance(device, queue, format);
...
pub(in crate::renderer) fn assert_gpu_scene_recovery_contract(
    ...
    objects::verify_scene_accounting(source_device, source_queue, recovery_device, recovery_queue);
    ...
    texture::assert_gpu_restored_material_policy(
        source_device,
        source_queue,
        recovery_device,
        recovery_queue,
    ...
    material_resource::assert_gpu_alpha_recovery(
        source_device,
        source_queue,
        recovery_device,
        recovery_queue,
    );
    lifetime::assert_lifetime_contract(
        source_device,
        source_queue,
        recovery_device,
        recovery_queue,
