...
- Optional host-supplied model normals and explicit `SurfaceLighting3d::Lambert`.
  The default `Unlit` path keeps the original color product. `Lighting3d` holds
  bounded ambient illumination and one optional directional source; no normals,
  simulation lights, shadows or physical materials are generated by Engine.
  Two-sided lighting flips back-facing normals. Nonuniform scale uses inverse
  transpose direction transport; interpolated normals normalize in the fragment.
- Opt-in `Fog3d` using camera-forward world distance for either projection.
  Fog changes RGB after lighting without changing alpha, depth, edge colors or
  submission counts; zero density disables it. Each material can opt out,
  independently of whether it uses lighting.
- Normal attributes join the retained/clipped, dynamic whole-bundle reuse,
  byte-budget and device-recovery contracts. Missing or unusable active Lambert
...
- Normal attribute/count errors and active-lighting validation extend the
